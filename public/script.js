...
// NOVA ROTA: DELETE para apagar um ativo
app.delete('/api/ativos/:id', async (req, res) => {
    const { id } = req.params;
    try {
        // O `ON DELETE CASCADE` na FK `ativo_pai_id` garante que os monitores
        // serão apagados automaticamente se a estação de trabalho (pai) for deletada.
